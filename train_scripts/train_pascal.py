...
def prep_data(ind,parameters,metadata,GT):    
    ...
    train_y_cl = {s:GT.get(s) for s in ind_train_X}    
    val_y_cl = {s:GT.get(s) for s in ind_val_X} 
...
def my_train():
    datapath = os.path.join(dataroot,'datasets/pascal/')
    pathgen = os.path.join(dataroot,'datasets/pascal/gen/') 
    ...
    logger.info('TRAINING - Pascal DATASET ') 
...
##############################################################################      
    ...
    with open(os.path.join(parameters['datagen'],'indexes.pkl'),'rb') as f:
    ...
    #gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction=gpu_p)     
